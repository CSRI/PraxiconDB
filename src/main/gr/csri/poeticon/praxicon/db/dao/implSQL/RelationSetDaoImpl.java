...
public class RelationSetDaoImpl extends JpaDao<Long, RelationSet>
    ...
    public RelationSet getRelationSetByName(String relationSetName) {
        ...
        Set<RelationSet> retrievedRelationSetsSet = new LinkedHashSet<>(query.
                getResultList());
        if (!retrievedRelationSetsSet.isEmpty()) {
            return retrievedRelationSetsSet.iterator().next();
        } else {
            return null;
        }
    }
    ...
    public RelationSet getRelationSet(RelationSet relationSet) {
        ...
        // 1. Retrieve relations from provided relation set
        Set<Relation> retrievedRelations = relationSet.getRelationsSet();
    ...
    public Set<RelationSet> getRelationSetsByRelationArgument(
        ...
        Query query = getEntityManager().createNamedQuery(
                "findRelationSetsByRelationArgument").setParameter(
                        "relationArgument", relationArgument);
        Set<RelationSet> relationSets = new LinkedHashSet<>(query.
                getResultList());
        return relationSets;
    }
    ...
    public Set<RelationSet> getRelationSetsByConcept(Concept concept) {
        ...
        Set<RelationSet> retrievedRelationSetsSet =
                getRelationSetsByRelationArgument(newRelationArgument);
        if (retrievedRelationSetsSet.isEmpty()) {
            return new LinkedHashSet<>();
        }
        return retrievedRelationSetsSet;
    }
    ...
    public Set<RelationSet> getRelationSetsByRelation(Relation relation) {
        ...
        Set<RelationSet> relationSets = new LinkedHashSet<>(query.getResultList());
        return relationSets;
    }
...
//        this.entityManager.flush();
    ...
    public Query getEntityQuery(RelationSet relationSet) {
        ...
        int i = 0;
        while (relationSet.getRelations().iterator().hasNext()) {
            i = i++;
            q.setParameter(i + 2, relationSet.getRelations().iterator().next());
        }
        return q;
    }
